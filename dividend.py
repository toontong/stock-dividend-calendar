...
环境变量 (.env 或系统环境):
  ...
  LLM_BASE_URL / LLM_MODEL / LLM_API_KEY   大模型连接信息（优先于 llm.json）
"""
...
# ═══════════════════════════════════════════════════════════
# 内置高分红蓝筹股 Top 100（无配置文件时的默认值）
# 筛选标准：A股主板 + 市值>500亿 + 近年稳定分红
# ═══════════════════════════════════════════════════════════
...
def load_config(config_dir: str | None) -> Config:
    ...
    llm_enabled = False
    llm_config: Optional[dict] = None
    llm_file = config_path / "llm.json"
    if llm_file.exists():
        llm_config = json.loads(llm_file.read_text(encoding="utf-8"))
